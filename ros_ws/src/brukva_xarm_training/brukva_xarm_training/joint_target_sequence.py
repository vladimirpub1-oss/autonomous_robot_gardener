...
class JointTargetSequence(Node):
    """Move the xArm base joint through a sequence of targets."""

    def __init__(self):
        super().__init__('joint_target_sequence')
        ...
        self.target_angles = [
            float(value)
            for value in self.get_parameter('target_angles').value
        ]
        self.current_target_index = 0
        self.target_angle = self.target_angles[0]
        ...
        self.get_logger().info(
            f'Sequence contains {len(self.target_angles)} targets'
        )
    ...
    def complete_current_target(
            self,
            actual_angle,
            error,
            actual_velocity,
    ):
        target_number = self.current_target_index + 1

        self.get_logger().info(
            f'TARGET {target_number} SUCCESS: '
        ...
        if self.current_target_index >= len(self.target_angles):
            self.finish(
                True,
                f'SEQUENCE COMPLETE: '
                f'{len(self.target_angles)}/'
                f'{len(self.target_angles)} targets reached',
            )
            return

        self.target_angle = self.target_angles[
